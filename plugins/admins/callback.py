...
from Monu .Mkg import get_string
from Monu.config import (AUTO_DOWNLOADS_CLEAR, BANNED_USERS,
                    SOUNCLOUD_IMG_URL, STREAM_IMG_URL,
                    TELEGRAM_AUDIO_URL, TELEGRAM_VIDEO_URL,
                    MUSIC_BOT_NAME, adminlist)
from Monu import YouTube, app
from Monu.core.call import Monu
from Monu.misc import SUDOERS, db
from Monu.utils import bot_sys_stats
from Monu.utils.database import (
    get_active_chats,
    get_lang,
    is_active_chat,
    is_music_playing,
...
from Monu.utils.formatters import seconds_to_min
from Monu.utils.inline import (
    stream_markup,
    stream_markup_timer,
    telegram_markup,
...
from Monu.utils.stream.autoclear import auto_clean
from Monu.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Monu.pause_stream(chat_id)
        ...
        await Monu.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await CallbackQuery.answer()
        await Monu.stop_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Monu.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await Monu.skip_stream(chat_id, link, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await Monu.skip_stream(chat_id, queued, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
